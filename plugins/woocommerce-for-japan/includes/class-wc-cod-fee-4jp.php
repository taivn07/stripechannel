...
class WooCommerce_Cod_Fee {
    ...
    public function __construct() {
        ...
        if( is_admin() ) {
	        $section_add = 'no';
	        if( version_compare( WC_VERSION, '2.6.0', '>' ) and isset($_GET['section']) and $_GET['section']=='cod'){
		        $section_add = 'yes';
	        }elseif( version_compare( WC_VERSION, '2.6.0', '>' ) and isset($_GET['section']) and $_GET['section']=='cod2'){
		        $section_add = 'yes';
	        }elseif( version_compare( WC_VERSION, '2.6.0', '<' ) and isset($_GET['section']) and $_GET['section']=='wc_gateway_cod'){
		        $section_add = 'yes';
	        }
//			if(isset($_GET['section']) and $_GET['section']=='wc_gateway_cod'){
			if(isset($_GET['section']) and $section_add == 'yes'){
    ...
    public function manage_form_fields() {
        ...
        $current_gateway    = !isset( $_GET['section'] ) || empty( $_GET['section'] ) ? '' : sanitize_text_field( urldecode( $_GET['section'] ) );
    ...
    public function manage_form_fields_for_basic_gateways( $current_gateway ) {
        ...
        ob_start() ?>
        <h3><?php _e( 'Extra charge for COD method', 'woocommerce-for-japan' ) ?></h3>
        <table class="form-table">
            <tbody>
                <tr valign="top">
                    <th scope="row" class="titledesc"><label for="woocommerce_cod_extra_charge_name"><?php _e( 'Fee name', 'woocommerce-for-japan' ) ?></label></th>
                    <td class="forminp">
                        <fieldset>
                            <legend class="screen-reader-text"><span><?php _e( 'Fee name', 'woocommerce-for-japan' ) ?></span></legend>
                            <input class="input-text regular-input " type="text" name="<?php echo $this->get_option_id( $current_gateway, 'name' ); ?>" id="woocommerce_cod_extra_charge_name" value="<?php echo $fee_name ?>" placeholder="<?php _e( 'COD Payment method fee', 'woocommerce-for-japan' ) ?>">
                        </fieldset>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row" class="titledesc"><label for="woocommerce_cod_extra_charge_amount"><?php _e( 'Extra charge amount', 'woocommerce-for-japan' ) ?></label></th>
                    <td class="forminp">
                        <fieldset>
                            <legend class="screen-reader-text"><span><?php _e( 'Extra charge amount', 'woocommerce-for-japan' ) ?></span></legend>
                            <input class="input-text regular-input " type="number" name="<?php echo $this->get_option_id( $current_gateway, 'amount' ); ?>" id="woocommerce_cod_extra_charge_amount" style="width:70px" value="<?php echo $charge_amount ?>" placeholder="0" min="0" step="1">
                        </fieldset>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row" class="titledesc"><label for="woocommerce_cod_extra_charge_max_cart_value"><?php _e( 'Maximum cart value to which adding fee', 'woocommerce-for-japan' ) ?></label></th>
                    <td class="forminp">
                        <fieldset>
                            <legend class="screen-reader-text"><span><?php _e( 'Maximum cart value for adding fee', 'woocommerce-for-japan' ) ?></span></legend>
                            <input class="input-text regular-input " type="number" name="<?php echo $this->get_option_id( $current_gateway, 'max_cart_value' ); ?>" id="woocommerce_cod_extra_charge_max_cart_value" style="width:70px" value="<?php echo $max_cart_value ?>" placeholder="0" min="0" step="1">
                            <p><?php _e( 'If you dont need this setting, please set empty, 0.', 'woocommerce-for-japan' ) ?></p>
                        </fieldset>
                    </td>
                </tr>
                <tr valign="top">
                    <th scope="row" class="titledesc"><?php _e( 'Includes taxes', 'woocommerce-for-japan' ) ?></th>
                    <td class="forminp">
                        <fieldset>
                            <legend class="screen-reader-text"><span><?php _e( 'Calculate taxes on this fee', 'woocommerce-for-japan' ) ?></span></legend>
                            <select name="<?php echo $this->get_option_id( $current_gateway, 'calc_taxes' ) ?>" id="woocommerce_cod_extra_charge_calc_taxes" style="" class="select">
                                <option value="no-tax" <?php selected( 'no-tax', $calc_taxes ) ?>><?php _e( 'Do not calculate taxes', 'woocommerce-for-japan' ) ?></option>
                                <option value="tax-incl" <?php selected( 'tax-incl', $calc_taxes ) ?>><?php _e( 'The fee is taxes included', 'woocommerce-for-japan' ) ?></option>
                                <option value="tax-excl" <?php selected( 'tax-excl', $calc_taxes ) ?>><?php _e( 'The fee is taxes excluded', 'woocommerce-for-japan' ) ?></option>
                            </select>
                        </fieldset>
                    </td>
                </tr>
...
//                $base_rate  = $tax->get_shop_base_rate();
            ...
            $do_apply                   = apply_filters( 'woocommerce_wc_pf_apply' , $do_apply , $extra_charge_amount , $subtotal , $current_gateway , $cart);
